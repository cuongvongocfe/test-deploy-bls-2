...
import partner5 from "../../../public/blogs/benhvienyhoccotruyen.png";
...
const Footer = () => {
  ...
  return (
    <footer className="mt-16 rounded-2xl bg-dark dark:bg-accentDark/90 m-2 sm:m-10 flex flex-col items-center text-light dark:text-dark">
      ...
      <ul className="flex flex- mt-8 justify-center  xs:justify-start">
        {SkillList.map((item, index) => {
          return (
            <li
              key={index}
              className="font-mono inline-block capitalize text-base xs:text-lg sm:text-xl  md:text-2xl py-0 xs:py-3 sm:py-4 lg:py-5 px-2 xs:px-6 sm:px-8 lg:px-12 border-2 border-solid border-dark dark:border-light rounded mr-3 mb-3 xs:mr-4 xs:mb-4  md:mr-6 md:mb-6 hover:scale-105 transition-all ease duration-200 cursor-pointer dark:font-normal" 
            >
              {item}
            </li>
          );
