...
// Tạo các đường dẫn tĩnh cho từng blog
...
export async function generateMetadata({ params }) {
  ...
  // Tạo danh sách hình ảnh cho Open Graph
...
export default function BlogPage({ params }) {
  ...
  // Làm sạch tag (xóa số) trước khi tạo liên kết
  const cleanedTag = removeNumbersFromTag(blog.tags[0]);
  const tagSlug = slug(cleanedTag);
      ...
      <article>
        ...
        <div className="mb-8 text-center relative w-full h-[70vh] bg-dark">
          <div className="w-full z-10 flex flex-col items-center justify-center absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2">
            <Tag
              name={cleanedTag} // Sử dụng tag đã làm sạch
              link={`/categories/${tagSlug}`} // Liên kết không chứa số
